...
double FitTurnOn(PdfFileHelper &PdfFile, TH1D *H1, TH1D *H2, double Target, bool tanh, bool noweight, bool Fix, bool Baseline)
{
   ...
   if(tanh == false)
   {
      ...
      if(tanh == false && Fix == false && Baseline == true)
      {
         double Sum = 0;
         int Count = 0;
         for(int i = G.GetN(); i > G.GetN() - 15 && i > 0; i--)
         {
            double x, y;
            G.GetPoint(i - 1, x, y);
            Sum = Sum + y;
   ...
   double Result95 = Find95(Fit, MinX, MaxX * 10, Target * Fit.Eval(10000));
   if(Fit.GetParError(0) != Fit.GetParError(0))
      Result95 = -1000;
   if(Fit.GetParError(1) != Fit.GetParError(1))
      Result95 = -1000;
   if(Fit.GetParError(2) != Fit.GetParError(2))
      Result95 = -1000;
   if(Fit.GetParError(3) != Fit.GetParError(3))
      Result95 = -1000;
